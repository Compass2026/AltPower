            ...
            <div className="px-6 py-8 flex flex-col gap-6">
              {navItems.map((item) => (
                <div key={item.name} className="flex flex-col gap-4">
                  <div className="text-[20px] font-black uppercase tracking-[0.2em] text-lime-primary border-b border-white/10 pb-2">
                    {item.name}
                  </div>
                  {item.dropdown ? (
                    <div className="grid grid-cols-1 gap-3 pl-4">
                      {item.dropdown.map((sub) => (
                        <Link
                          ...
                          className="text-[24px] font-bold uppercase tracking-widest text-white/60 hover:text-white py-2"
                        >
                          {sub.name}
                        </Link>
                      ))}
                    ...
                    <a
                      href={item.href}
                      onClick={() => setIsMobileMenuOpen(false)}
                      className="text-[24px] font-bold uppercase tracking-widest text-white/60 hover:text-white pl-4 py-2"
                    >
                      View Page
                    </a>
                  )}
              ...
              <a
                href="tel:+16364977314"
                className="flex items-center gap-3 text-[24px] font-black uppercase tracking-widest text-white/80 pt-4 border-t border-white/10"
              >
                <Phone className="w-6 h-6 text-lime-primary" />
                (636) 497 7314
              </a>
            </div>
          </motion.div>
